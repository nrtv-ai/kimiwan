...
class TaskOrchestrator {
    ...
    assign(taskId, agentId) {
        ...
        this.messageBus.send(task.createdBy, 'task_request', {
            kind: 'task_request',
            task: {
                type: task.type,
     ...
     */
    assignToCapableAgent(taskId, capabilities) {
    ...
    complete(taskId, result) {
        ...
        if (task.assignedTo) {
            this.messageBus.send(task.assignedTo, 'task_response', {
                kind: 'task_response',
                taskId,
                result,
            }, task.createdBy);
     ...
     * Create a subtask
     */
    createSubtask(parentTaskId, request, createdBy) {
        const parent = this.tasks.get(parentTaskId);
        if (!parent)
            return undefined;
        const subtask = this.create({ ...request, parentTaskId, contextId: parent.contextId }, createdBy);
    ...
    checkParentCompletion(parentTaskId) {
        ...
        const allDone = subtasks.every(t => t.status === 'completed' || t.status === 'failed' || t.status === 'cancelled');
