#!/usr/bin/env python3
"""Debug vote API - check what the API expects"""
import requests
import json
import hashlib
import time
...
def get_pow_headers():
    ...
    if 'challenge' not in challenge_data:
        print(f"Unexpected challenge response: {challenge_data}")
        time.sleep(1)
...
def vote_post(post_id, vote_payload):
    """Vote on a post."""
    ...
    print(f"Testing payload: {vote_payload}")
